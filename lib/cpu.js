...
var cpu = {
	...
	step: function() {
		...
		switch(cpu.state) {
			...
			case 8: //The *opcode* 0001 means add the value in the *Accumulator* register to the data stored in memory at the address specified by the *operand*"
				setState(81, "Decode", "The *Decode Unit* sends the *operand* to the *Memory Address Register* which gets copied onto the *Address Bus*");
	...
	updateAnnotations: function() {
		...
		connect({e:CPU, h:"right", v:"5%"}, {e: RAM, h:"left"}, {"stroke-width": 30, "stroke": "#F00", "arrow-end":"block-narrow-short"}, "Address bus");
		connect({e:CPU, h:"right", v:"56%"}, {e: RAM, h:"left"}, {"stroke-width": 30, "stroke": "#F00", "arrow-end":"block-narrow-short", "arrow-start": "block-narrow-short"}, "Data bus");
		connect({e:CPU, h:"right", v:"95%"}, {e: RAM, h:"left"}, {"stroke-width": 30, "stroke": "#F00", "arrow-end":"block-narrow-short", "arrow-start": "block-narrow-short"}, "Control bus");
	},
	...
	init: function(jqCPU) {
		...
		function getRegisterHtml(name, value, desc) {
			return '<div class="register" id="reg_' + name.toLowerCase()+'"><div class="reg_name">' + desc +' ('+name+')' + '</div><div id="reg_' + name.toLowerCase() + '_val" class="reg_val value value_denary editable" data-description="' + desc + '">' + value + '</div></div>';
		}
